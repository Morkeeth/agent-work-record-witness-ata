...
where Firestore and application-token-gated APIs hold the record (Cloud Run is public at
the IAM layer by design; the 401 is the bearer token, not IAM). ADK + Vertex Gemini
explain HOLD decisions for humans; they never override a probe. Cold eligibility: 1/3 with
`pip install -r requirements.txt` and no GCP credentials; 0/3 on bare stock Python with no
gateway deps; 3/3 on the deployed path with ADC — all three measured, not claimed.
```
