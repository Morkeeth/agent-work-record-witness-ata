...
**THE GATE.** An advisory clearance check (`verify-claims`) reads a pull request body, probes each
claim against the object, and fails red when the object disagrees. Install is a YAML file. Five
minutes. (Not a "required check" while branch protection is off.)
